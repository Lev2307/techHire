...
KEYWORDS_END_OF_SECTIONS = (
    ...
    r'Жд[её]м Вашего откликаю.*?|Welcome to.*|Если наша вакансия тебе откликнулась.*|Присоединяйтесь к нам.*'
)
...
def get_applicant_criterias_for_filtering_vacancies(user: Applicant) -> dict:
    ...
    applicant_data = {
        ...
        'preferred_work_format': " ".join([wf.name for wf in user.preferred_work_format.all()]),
        'specializations': " ".join([spec.name for spec in user.specializations.all()]),
        'technologies': " ".join([technology.name for technology in user.technologies.all()]),
    }
...
def get_applicant_favourite_vacancies_info_for_filtering_vacancies(vacancies: QuerySet[Vacancy]) -> dict:
    ...
    for vacancy in vacancies:
        ...
        duties = "" if vacancy.duties == NOT_FOUND_DUTIES else vacancy.duties
        reqs = "" if vacancy.requirements == NOT_FOUND_REQS else vacancy.requirements
        work_cond = "" if vacancy.working_conditions == NOT_FOUND_WORK_COND else vacancy.working_conditions

        applicant_fav_vacancies_data[vacancy.id] = {
            'title': vacancy.title,
            'vacancy_texts': duties + reqs + work_cond,
            ...
            'work_format': " ".join([wf.name for wf in vacancy.work_format.all()]),
        }

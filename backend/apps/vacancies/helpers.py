...
KEYWORDS_END_OF_SECTIONS = (
    ...
    r'Жд[её]м Вашего откликаю.*?'
)

def extract_duties_requirements_working_conditions_by_keywords(text):
    text = text.replace('\u200b', '').replace('<mark>', '').replace('</mark>', '').replace('<strong>', '').replace('</strong>', '').replace('<em>', '').replace('</em>', '').replace('<span>', '').replace('</span>', '').replace('—', '').replace('_', '').replace('·', '').replace('●', '')
...
def get_applicant_criterias_for_filtering_vacancies(user: Applicant) -> dict:
    ...
    applicant_data = {
        ...
        'preferred_work_format': " ".join([wf.name for wf in user.preferred_work_format]),
        'specializations': " ".join([spec.name for spec in user.specializations]),
        'technologies': " ".join([technology.name for technology in user.technologies]),
    }
...
def get_applicant_favourite_vacancies_info_for_filtering_vacancies(vacancies: QuerySet[Vacancy]) -> dict:
    ...
    for vacancy in vacancies:
        ...
        applicant_fav_vacancies_data[vacancy.id] = {
            ...
            'working_conditions': vacancy.working_conditions,
            ...
            'work_format': " ".join([wf.name for wf in vacancy.work_format]),
        }
...
def get_applicant_search_history_info_for_filtering_vacancies(search_history: QuerySet[SearchHistory]):
    ...
    applicant_search_history_data = {}
    for sh in search_history:
        applicant_search_history_data[sh.id] = sh.search_query
    return applicant_search_history_data